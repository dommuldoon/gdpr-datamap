...
describe("GridView — systemType layout", () => {
  it("renders a column header for each unique system type", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Integration" })
    ).toBeInTheDocument();
    expect(
  ...
  it("places each system under its correct column", () => {
    ...
    const appCol = screen
      ...
      .closest(".flex")!;
    expect(
      within(appCol as HTMLElement).getByText("Storefront")
    ).toBeInTheDocument();
    ...
    const dbCol = screen
      ...
      .closest(".flex")!;
    expect(
      within(dbCol as HTMLElement).getByText("App Database")
    ).toBeInTheDocument();
    expect(
      within(dbCol as HTMLElement).getByText("Search Engine")
    ).toBeInTheDocument();
    ...
    const intCol = screen
      ...
      .closest(".flex")!;
    expect(
      within(intCol as HTMLElement).getByText("Stripe")
    ).toBeInTheDocument();
  });
  ...
  it("shows correct system count label in each column header", () => {
    ...
    expect(
      screen.getByText("2 systems", { selector: "p" })
    ).toBeInTheDocument();
  });
  ...
  it("renders only columns for types present in the systems list", () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "Integration" })
    ).not.toBeInTheDocument();
  });
...
describe("GridView — dataUse layout", () => {
  it("renders a column for each unique data use", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "advertising.third_party" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "improve.system" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "provide.system" })
    ).toBeInTheDocument();
  });
...
describe("GridView — layout switch", () => {
  it("re-renders correct columns when layoutMode prop changes", () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "provide.system" })
    ).not.toBeInTheDocument();
  ...
  it("re-renders when systems prop changes", () => {
    ...
    rerender(
      <GridView
        systems={[storefront, stripe]}
        layoutMode="systemType"
        showArrows={false}
      />
    );
